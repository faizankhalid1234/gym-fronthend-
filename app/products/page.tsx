...
export default function ProductsPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-primary-50/40 py-6 sm:py-8 md:py-10 px-4 sm:px-6">
      <div className="max-w-7xl mx-auto">
        <div className="rounded-3xl bg-gradient-to-r from-primary-900 via-primary-800 to-primary-700 text-white p-6 sm:p-8 mb-8 shadow-xl">
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold">Gym Products</h1>
          <p className="mt-2 text-primary-100 max-w-2xl">Premium equipment and supplements curated for serious training.</p>
        </div>
        ...
        <div className="mb-6 sm:mb-8 flex flex-wrap gap-2 sm:gap-3 md:gap-4">
          {categories.map((category) => (
            <button
              ...
              className={`px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 rounded-full font-semibold text-sm sm:text-base transition-all duration-300 ${
                selectedCategory === category
                  ? 'bg-primary-600 text-white shadow-lg ring-4 ring-primary-200'
                  : 'bg-white/90 backdrop-blur text-gray-700 hover:bg-white shadow-md hover:shadow-lg'
              }`}
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
            {filteredProducts.map((product) => (
              <Link
                ...
                className="bg-white/95 rounded-2xl border border-slate-100 shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden group transform hover:-translate-y-2"
              >
                <div className="relative h-48 sm:h-56 md:h-64 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">
                  ...
                  <p className="text-gray-600 text-xs sm:text-sm mb-3 line-clamp-2">{product.description}</p>
                    ...
                    <span className="text-xs sm:text-sm text-primary-700 capitalize bg-primary-50 px-2 py-1 rounded-full">{product.category}</span>
                  </div>
                  {product.rating && (
